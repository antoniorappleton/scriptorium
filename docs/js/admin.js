async function loadCiclos() {
  ...
  el.innerHTML = '<option value="">-- seleccionar --</option>' + (data || []).map(c => `<option value="${c.id}">${c.nome}</option>`).join('');
}
...
async function refreshTurmas() {
  ...
  const { data, error } = await window.supabase.from('turmas').select('*, ciclos(*)').order('nome');
  if (error) return (el.innerHTML = 'Erro ao carregar turmas: ' + error.message);
  el.innerHTML = (data || []).map(t => `<div class="p-2 border mb-1">${t.nome} (Ano ${t.ano || ''}) — Ciclo: ${t.ciclos?.nome || ''}</div>`).join('');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('createTurma').addEventListener('click', async () => {
    ...
    const { error } = await window.supabase.from('turmas').insert([{ nome, ano, ciclo_id }]);
  ...
  document.getElementById('importCsv').addEventListener('click', async () => {
    ...
    Papa.parse(f, {
      ...
      complete: async (results) => {
        ...
        document.getElementById('importStatus').textContent = 'A inserir ' + rows.length + ' alunos...';
        ...
        document.getElementById('importStatus').textContent = 'Importação concluída: ' + rows.length + ' alunos';
        await refreshTurmas();
      }
    });
  });
