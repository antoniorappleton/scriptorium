...
async function sincronizarPendentes() {
  ...
  for (const p of pendentes) {
    try {
      ...
      const { error } = await window.supabase.from("ocorrencias").insert([payload]);
      if (error) throw error;
    } catch (err) {
...
async function loadAlunos() {
  ...
  if (!list) return;
  list.innerHTML = "<div class='admin-list-item' style='color: var(--text-muted);'>A carregar...</div>";
  try {
    const { data, error } = await window.supabase
      .from("alunos")
      .select("*")
    ...
    if (error) throw error;
    if (!data || !data.length) {
      list.innerHTML = "<div class='admin-list-item' style='color: var(--text-muted);'>Sem alunos adicionados.</div>";
      return;
    }
    list.innerHTML = data
      .map(
...
async function carregarOcorrencias(q = "") {
  ...
  listEl.innerHTML = "<div style='text-align: center; color: var(--text-muted); padding: 20px;'>A carregar...</div>";
  try {
    ...
    const pendentes = JSON.parse(localStorage.getItem("pendentes") || "[]");
    const rows = pendentes.map((p) => ({ ...p, _local: true })).concat(data || []);
    ...
    if (!rows.length) {
      ...
      listEl.innerHTML = rows
        .map((r) => {
          ...
          const dataVal = r.data || (r.created_at ? r.created_at.slice(0, 10) : new Date().toISOString().slice(0, 10));
          const formattedDate = dataVal.split('-').reverse().join('/'); // DD/MM/YYYY
          const badgeClass = r._local ? 'badge badge-pending' : 'badge badge-synced';
    ...
    if (pendentes.length) {
      ...
      listEl.innerHTML = "<div style='text-align: center; color: var(--error); padding: 20px;'>Erro ao carregar ocorrências.</div>";
    }
  }
}
    ...
    if (isAdmin) {
      ...
      if (addForm) {
        addForm.addEventListener("submit", async (e) => {
          ...
          const turma = document.getElementById("alunoTurmaAdmin")?.value?.trim();
          const ciclo = document.getElementById("alunoCicloAdmin")?.value?.trim();
